// components/DeliveryNoteLineItemsEditor.tsx — Simplified line items: Description + Qty only
"use client";

import { useDeliveryNoteStore, type DeliveryNoteLineItem } from "@/lib/store/deliveryNoteStore";

export function DeliveryNoteLineItemsEditor() {
  const items = useDeliveryNoteStore((s) => s.items);
  const addItem = useDeliveryNoteStore((s) => s.addItem);
  const updateItem = useDeliveryNoteStore((s) => s.updateItem);
  const removeItem = useDeliveryNoteStore((s) => s.removeItem);

  return (
    <div className="space-y-3">
      {/* Header row — no Rate or Amount */}
...
function DeliveryNoteLineItemRow({
  item,
  index,
  onUpdate,
  onRemove,
  ...
  return (
    <div className="group rounded-lg border border-mist p-3 md:p-0 md:border-0">
      <div className="grid grid-cols-12 gap-2 items-start">
        ...
        {/* Delete */}
        <div className="col-span-4 md:col-span-1 flex items-center justify-center py-2">
          {canRemove && (
            <button
