// components/InvoicePreview.tsx — Live read-only invoice preview
"use client";

import { useInvoiceStore, type InvoiceParty } from "@/lib/store/invoiceStore";
  ...
  return (
    <div className="relative bg-white rounded-xl shadow-soft border border-mist overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 overflow-hidden">
        <div
          className="text-[80px] sm:text-[100px] font-bold text-ink/[0.03] tracking-widest select-none"
      ...
      <div className="relative z-0 p-6 sm:p-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right text-sm text-ink/60">
            ...
            <p>Issued: {formatDate(store.issueDate)}</p>
            <p>Due: {formatDate(store.dueDate)}</p>
          </div>
        </div>
...
function PartyBlock({ party }: { party: InvoiceParty }) {
  ...
  return (
    <div className="space-y-0.5 text-ink/70">
      ...
      {party.mobile && <p>Mobile: {party.mobile}</p>}
      {party.fax && <p>Fax: {party.fax}</p>}
