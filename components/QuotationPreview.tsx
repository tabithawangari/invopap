// components/QuotationPreview.tsx — Live read-only quotation preview (NO TAX)
"use client";
      ...
      <div className="relative z-0 p-6 sm:p-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <h1
              ...
              style={{ color: accent }}
            >
              {store.documentTitle || "QUOTATION"}
            </h1>
          </div>
          <div className="text-right text-sm text-ink/60">
            <p className="font-medium text-ink">
              {store.quotationNumber || "—"}
            </p>
            <p>Date: {formatDate(store.quotationDate)}</p>
            {store.validUntil && (
              <p>Valid Until: {formatDate(store.validUntil)}</p>
            )}
          </div>
        </div>
        ...
        {store.termsAndConditions && (
          <div className="text-sm text-ink/60 border-t border-mist pt-4">
            ...
            <p className="whitespace-pre-wrap">{store.termsAndConditions}</p>
          </div>
        )}
