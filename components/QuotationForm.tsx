// components/QuotationForm.tsx — Editable quotation form (NO tax, has T&C + notes + validUntil)
"use client";
...
export function QuotationForm() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {/* ─── Quotation by / Quotation to — side by side ─── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
        <PartySection title="Quotation by" prefix="from" />
        <PartySection title="Quotation to" prefix="to" />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
        <div>
          ...
          <input
            type="text"
            value={store.quotationNumber}
            onChange={(e) => store.setField("quotationNumber", e.target.value)}
        ...
        <div>
          ...
          <input
            type="date"
            value={store.quotationDate}
            onChange={(e) => store.setField("quotationDate", e.target.value)}
            className="input"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) => store.setField("validUntil", e.target.value)}
            className="input"
          />
        </div>
      </div>
      ...
      <section className="space-y-2">
        ...
        <textarea
          ...
          onChange={(e) => store.setField("termsAndConditions", e.target.value)}
          rows={3}
          maxLength={2000}
          placeholder="Payment terms, validity conditions, delivery timeline..."
          className="input resize-none"
        />
        <p className="text-xs text-ink/30 text-right">{store.termsAndConditions.length}/2000</p>
      </section>
      ...
      <section className="space-y-2">
        ...
        <textarea
          ...
          rows={3}
          maxLength={2000}
          placeholder="Notes — any relevant information not covered above..."
          className="input resize-none"
        />
...
function PartySection({
  ...
  const party = useQuotationStore((s) => s[prefix]);
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="space-y-3">
        {field("name", "Name", prefix === "from" ? "Your Business Name" : "Client Name")}
