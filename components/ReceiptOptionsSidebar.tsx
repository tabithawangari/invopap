// components/ReceiptOptionsSidebar.tsx — Right sidebar for Receipt: type, accent color, currency
"use client";
...
export interface ReceiptOptionsSidebarProps {
  onDownload: () => void;
  onNew: () => void;
  saving: boolean;
}

export function ReceiptOptionsSidebar({
  onDownload,
  onNew,
  saving,
}: ReceiptOptionsSidebarProps) {
  const { accentColor, currency, setAccentColor, setCurrency } =
    useReceiptStore();
  ...
  return (
    <div className="space-y-5 p-4 bg-white rounded-xl border border-mist">
      ...
      {/* ─── Document Type ─── */}
      <DocumentTypeSwitcher variant="sidebar" fallbackLabel="Receipt" />
